...
app.get('/api/articles/latest', async (req, res) => {
    try {
        const articles = await Article.find()
            .sort({ createdAt: -1 })
            .limit(6)
            // Include views so homepage/article lists can show correct view counts
    ...
    try {
        ...
        const [articles, total] = await Promise.all([
            Article.find()
                ...
                .skip(skip)
                .limit(limit)
                // Include views so articles.html list shows real view counts
                .select('title slug author publicationDate abstract coverImage volume issue year doi license keywords authors pages section views issueId'),
